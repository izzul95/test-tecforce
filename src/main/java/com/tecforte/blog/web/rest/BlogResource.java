...
public class BlogResource {
    ...
    /**
     * {@code DELETE  /blogs/clean} : delete all Blog entries by keyword.
     *
    ...
    public ResponseEntity<Void> deleteEntryBlogByKeyword(@RequestBody String keyword) {
        log.debug("REST request to delete all Blog entries by keyword {}", keyword);
        entryService.deleteByKeyword(keyword);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, keyword.toString())).build();
    }

    /**
     * {@code DELETE  /blogs/{id}/clean} : delete entries in one blog by keyword.
     *
     * @param keyword the keyword to delete.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    ...
    public ResponseEntity<Void> deleteEntryBlogByKeywordAndBlogId(@PathVariable Long id ,@RequestBody String keyword) {
        log.debug("REST request to delete entries in one blog {} by keyword {}", id ,keyword);
        entryService.deleteByKeywordWithId(id , keyword);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, keyword.toString())).build();
    }
}
